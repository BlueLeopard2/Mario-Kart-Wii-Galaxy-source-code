...
void LoadLeftPurpleSparkEffects(Pulsar::Race::ExpPlayerEffects& effects, EGG::Effect** effectArray, u32 firstEffectIndex, u32 lastEffectIndex, const Mtx34& playerMat2, const Vec3& wheelPos, bool updateScale) {
    const u32 smtCharge = effects.kartPlayer->pointers.kartMovement->smtCharge;
    if(smtCharge >= 550 && System::sInstance->IsContext(Pulsar::PULSAR_UMTS)) {
...
void LoadRightPurpleSparkEffects(Pulsar::Race::ExpPlayerEffects& effects, EGG::Effect** effectArray, u32 firstEffectIndex, u32 lastEffectIndex, const Mtx34& playerMat2, const Vec3& wheelPos, bool updateScale) {
    const u32 smtCharge = effects.kartPlayer->pointers.kartMovement->smtCharge;
    if(smtCharge >= 550 && System::sInstance->IsContext(Pulsar::PULSAR_UMTS)) {
...
void CreateUMT(Kart::Movement& movement) {
    bool isUMTs = Pulsar::System::sInstance->IsContext(Pulsar::PULSAR_UMTS);
