...
const GameMode gameMode = Racedata::sInstance->menusScenario.settings.gamemode;
    if(System::sInstance->IsContext(PULSAR_FEATHER) && System::sInstance->IsContext(PULSAR_FLYINGBLOOP) == Pulsar::MKWGSETTING_FLYINGBLOOP_FEATHER || mode == MODE_TIME_TRIAL || mode == MODE_GHOST_RACE) {
...
const GameMode mode = scenario.settings.gamemode;
const GameMode gameMode = Racedata::sInstance->menusScenario.settings.gamemode;
    if (Settings::Mgr::Get().GetUserSettingValue(Settings::SETTINGSTYPE_MKWG2, SETTINGMKWG_INVIS_WALLS) == Pulsar::MKWGSETTING_INVISWALLS_DISABLED) {
        acceptedFlags = static_cast<KCLBitfield>(acceptedFlags & ~(1 << KCL_INVISIBLE_WALL));
    }
    else if(System::sInstance->IsContext(PULSAR_FEATHER) && System::sInstance->IsContext(PULSAR_FLYINGBLOOP) == Pulsar::MKWGSETTING_FLYINGBLOOP_FEATHER || mode == MODE_TIME_TRIAL || mode == MODE_GHOST_RACE) {
...
const GameMode mode = scenario.settings.gamemode;
const GameMode gameMode = Racedata::sInstance->menusScenario.settings.gamemode;
    if(System::sInstance->IsContext(PULSAR_FEATHER) && System::sInstance->IsContext(PULSAR_FLYINGBLOOP) == Pulsar::MKWGSETTING_FLYINGBLOOP_FEATHER || mode == MODE_TIME_TRIAL || mode == MODE_GHOST_RACE) timer = 0;
    else timer--;
    return timer;
}
...
const GameMode mode = scenario.settings.gamemode;
const GameMode gameMode = Racedata::sInstance->menusScenario.settings.gamemode;
    if(System::sInstance->IsContext(PULSAR_FEATHER) && System::sInstance->IsContext(PULSAR_FLYINGBLOOP) == Pulsar::MKWGSETTING_FLYINGBLOOP_FEATHER || mode == MODE_TIME_TRIAL || mode == MODE_GHOST_RACE) objKumo.LoadGraphics("feather.brres", mdlName, shadowSrc, 0, 0,
...
void ConditionalNoBlooperAnimation(ModelDirector* mdl, u32 id, g3d::ResFile& brres, const char* name, AnmType type, bool hasBlend,
    ...
    if(System::sInstance->IsContext(PULSAR_FLYINGBLOOP) != Pulsar::MKWGSETTING_FLYINGBLOOP_FEATHER) {
