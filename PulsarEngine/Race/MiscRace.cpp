...
static void NonGhostPlayerCount(RacedataScenario& scenario, u8* playerCount, u8* screenCount, u8* localPlayerCount) {
    ...
    isInTimeTrial = (scenario.settings.gamemode == MODE_TIME_TRIAL);
...
const char* ChangeItemWindowPane(ItemId id, u32 itemCount) {
    const bool feather = (System::sInstance->IsContext(PULSAR_FEATHER) && System::sInstance->IsContext(PULSAR_FLYINGBLOOP) == Pulsar::MKWGSETTING_FLYINGBLOOP_FEATHER);
    const bool megaTC = (Pulsar::Settings::Mgr::Get().GetUserSettingValue(Pulsar::Settings::SETTINGSTYPE_MKWG2, Pulsar::SETTINGMKWG_THUNDERCLOUD) == Pulsar::MKWGSETTING_THUNDERCLOUD_MEGA);
    const char* paneName;
    if (id == BLOOPER && (feather || isInTimeTrial)) {
        if (itemCount == 2) paneName = "feather_2";
        else if (itemCount == 3) paneName = "feather_3";
        else paneName = "feather";
    }
    else if ((id == THUNDER_CLOUD && megaTC && RKNet::Controller::sInstance->roomType != RKNet::ROOMTYPE_VS_WW) || (id == THUNDER_CLOUD && (RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_VS_REGIONAL || RKNet::Controller::sInstance->roomType == RKNet::ROOMTYPE_BT_REGIONAL))) paneName = "megaTC";
