...
//Overwrites CC rules -> 10% 100, 65% 150, 25% mirror and/or in frooms, overwritten by host settingg
static void DecideCC(ExpSELECTHandler& handler) {
    ...
    if (!system->IsContext(PULSAR_CT)) reinterpret_cast<RKNet::SELECTHandler&>(handler).DecideEngineClass();
        ...
        const u8 ccSetting = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_HOST, SETTINGHOST_CC);
        RKNet::Controller* controller = RKNet::Controller::sInstance;
